            ...
            fetch('/api/Participant/check')
                ...
                .then(data => {
                    if (data.authenticated) {
                        ...
                        if (data.hasSpun) {
                            // If we can get ID from data?
                            // The check API doesn't return ID directly for security (as per plan), 
                            // but for redirection we might need it OR just redirect to /status (and let status page fetch self).
                            // Wait, status page expects ?id=... currently.
                            // We should probably update status page to not need ID, OR return ID in check for this specific case.
                            // Let's rely on the fact that if they are authenticated, they can access /status.
                            // But front-end URL needs ID? 
                            // I'll update status page to be auto-resolving too.
                            // For now, let's just use what we have. 
                            // Check API returns: name, phoneMasked, hasSpun, prize, redeemCode, redeemStatus
                            // It does NOT return ID.
                            // Update: Use `window.location.href = '/status'` and let Status Page handle it?
                            // SpinWheel currently redirects to `/status?id=${pid}`.
                            
                            // Let's fix this redirection issue.
                            window.location.href = '/status' 
                            return
                        }
                ...
                */}
                ...
                <div className="relative flex flex-col items-center justify-center min-h-screen w-full bg-[url('/x-max-bg.jpg')] object-cover bg-cover bg-center overflow-hidden"
                    ...
                    <ClaimPrizeModal
                        ...
                        onClose={() => window.location.href = '/status'} 
                    />
