...
export default function AdminDashboard() {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    // For pending or other non-final states (if any), direct update
    // But per requirements, we are mostly moving FROM pending TO final.
    // If strict one-way, maybe we don't allow moving back to pending? 
    // Assuming 'pending' -> 'claimed'/'rejected' is the main flow.
  ...
  const handleResetDb = async () => {
    try {
      const res = await fetch("/api/sys_9e4a/reset", { method: "POST" });
      if (res.ok) {
      ...
      <div className="min-h-screen bg-gray-50 p-8 text-black relative pt-24">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-end items-center mb-8 gap-4">
            {/* <button
              onClick={() => setResetDbModal(true)}
              className="bg-gray-800 text-white px-4 py-2 rounded hover:bg-gray-900 transition shadow-md flex items-center gap-2"
            >
              <span>⚠️</span> Reset Database
            </button> */}
        ...
        {/* {resetDbModal && (
          <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200 border-2 border-red-500">
              <h3 className="text-xl font-bold text-red-600 mb-2">DANGER ZONE</h3>
              <div className="text-gray-800 mb-6 text-sm font-medium">
                You are about to <span className="underline font-bold">DELETE ALL DATA</span>.
                <br /><br />
                This will:
                <ul className="list-disc pl-5 mt-2 text-xs text-gray-600">
                  <li>Delete ALL participants</li>
                  <li>Clear ALL redeem records</li>
                  <li>Reset Prize Inventory to defaults</li>
                </ul>
                <br />
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-red-600 text-white font-bold rounded hover:bg-red-700 transition shadow-lg"
                >
